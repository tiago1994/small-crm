<div>
            ...
            <table class="w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 overflow-hidden mb-2">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 text-center w-32">{{ $offer->offers()->count() }}</td>
                        <td class="px-6 py-4 text-center w-32">{{ $offer->offers()->where('value', '!=', null)->where('description', '!=', null)->count() }}</td>
                        <td class="px-6 py-4 w-32">{{ \Carbon\Carbon::parse($offer->created_at)->format('d/m/Y H:i:s') }}</td>
                        <td class="px-6 py-4 text-center w-32">
                            <a href="{{asset('storage/offers/'.$offer->file)}}" target="_blank"><i class="la la-download"></i></a>
                            <a href="{{route('offers-list', $offer->id)}}" class="ml-1"><i class="la la-list"></i></a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Deletar Cotação</h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        Deletando esta cotação você ira perder todo o histórico da mesma.
                                    </p>
                                </div>
                            </div>
                        </div>
    ...
    @livewire('components.offer-modal')
</div>